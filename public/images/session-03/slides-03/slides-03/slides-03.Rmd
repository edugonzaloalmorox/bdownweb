...
scott = import("/Users/Personas/Dropbox/PhD/teaching/applied_eco_r/session-03/data/raw/edudata.dta")
 ...
 # prepare data
 class1 =  scott %>% group_by(class1, class2, class3) %>% tabyl(class1) %>% mutate(class = 1) %>% rename(category = class1)
 class2 =  scott  %>% tabyl(class2) %>% mutate(class = 2) %>% rename(category = class2)
 class3 =  scott %>% tabyl(class3) %>% mutate(class = 3) %>% rename(category = class3)
 
 # link datasets by row
 df = rbind(class1, class2, class3) %>% mutate(class = as.factor(class))
 
 
 # plot
 a = ggplot(df, aes(category, percent, fill = class)) + geom_bar(stat="identity")
 a
